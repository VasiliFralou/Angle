...
class ResultFragment : Fragment(R.layout.list_result_fragment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        resultsVM.resultsLive.observe(activity as MainActivity) { list ->
            listResults.clear()
            listResults.addAll(list)
            listResults.sortByDescending { it.pointResult }
            binding.resultsListRV.adapter?.notifyDataSetChanged()
        }

        val adapter = ResultsListAdapter(listResults)
        binding.resultsListRV.adapter = adapter
        binding.resultsListRV.layoutManager = LinearLayoutManager(activity as MainActivity)
    }
}